...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    for (i, (label, configured)) in providers.iter().enumerate() {
        ...
        let status = if *configured { app.t("menu.sync.configured") } else { app.t("menu.sync.not_configured") };
        ...
        if i == 3 && app.menu_cursor == 3 {
            ...
            lines.push(Line::from(Span::styled(
                ...
                Style::default().fg(theme.category_fg).add_modifier(Modifier::BOLD),
            )));

            let interval_str = format!("{} min", app.settings.sync_interval_mins);
            let notion_items = vec![
                (app.t("menu.settings.notion_key").to_string(), notion_key_display.clone()),
                (app.t("menu.settings.notion_db").to_string(), notion_db_display.clone()),
                (app.t("menu.settings.sync_interval").to_string(), interval_str),
            ];
            for (label, value) in notion_items {
                lines.push(Line::from(vec![
                    Span::raw(format!("     {}: ", label)),
                    Span::styled(value, Style::default().fg(theme.accent_secondary).add_modifier(Modifier::BOLD)),
    ...
    lines.push(Line::from(Span::styled(
        format!(" [Enter] {} │ [Esc] {}", app.t("hint.edit"), app.t("confirm.no")),
        Style::default().fg(theme.muted),
    )));
