...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let detail_style = if detail_active { active_style } else { inactive_style };
    ...
    if let Some(task) = app.current_task() {
        ...
        lines.push(Line::from(vec![Span::raw(format!(" {}   : ", app.t("section.review_plan"))), Span::styled(&task.custom_review_str, Style::default().fg(theme.muted))]));
        
        if !task.review_subtasks.is_empty() {
            lines.push(Line::from(""));
            lines.push(Line::from(vec![Span::styled(format!(" --- {} ---", app.t("section.review_plan")), Style::default().fg(theme.category_fg))]));
            for sub in &task.review_subtasks {
                ...
                if sub.completed {
                    lines.push(Line::from(Span::styled(line_str, Style::default().bg(theme.success).fg(theme.bg))));
                } else {
        ...
        let scroll_indicator = if detail_active && app.detail_scroll_offset > 0 {
            format!(" {} [3] ▲{}", app.t("section.info"), app.detail_scroll_offset)
        } else {
            format!(" {} [3] ", app.t("section.info"))
        };
        
        let header = Paragraph::new(lines)
            .block(Block::default().borders(Borders::ALL).title(scroll_indicator).border_style(detail_style).style(Style::default().bg(theme.bg)))
            .scroll((app.detail_scroll_offset, 0));
        f.render_widget(header, layout[0]);

        let description = Paragraph::new(task.description.as_str())
            .block(Block::default().borders(Borders::ALL).title(format!(" {} [d/e] ", app.t("section.description"))).border_style(detail_style).style(Style::default().bg(theme.bg).fg(theme.fg)));
        ...
        let importance = Paragraph::new(importance_str)
            .block(Block::default().borders(Borders::ALL).title(format!(" {} ", app.t("section.importance"))).border_style(detail_style).style(Style::default().bg(theme.bg).fg(imp_color)));
        f.render_widget(importance, bottom_layout[0]);

        let subtasks_text = format!(" {}: {} ('r')", app.t("section.review_plan"), task.custom_review_str);
        
        let rev_box = Paragraph::new(subtasks_text)
            .block(Block::default().borders(Borders::ALL).title(format!(" {} ", app.t("section.review_plan"))).border_style(detail_style).style(Style::default().bg(theme.bg).fg(theme.fg)));
        f.render_widget(rev_box, bottom_layout[1]);

    } else {
        let empty = Paragraph::new(app.t("detail.empty"))
            .block(Block::default().borders(Borders::ALL).title(format!(" {} [3] ", app.t("section.info"))).border_style(detail_style).style(Style::default().bg(theme.bg).fg(theme.muted)));
