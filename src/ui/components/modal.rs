...
pub fn draw(f: &mut Frame, app: &mut App) {
        ...
        let mut text = match app.input_mode {
            ...
            InputMode::ConfirmingUpdate => {
                let version = app
                    .update_info
                    .as_ref()
                    .map_or("?".to_string(), |i| i.latest.clone());
                ...
                f.render_widget(
                    Paragraph::new(text)
                        .block(
                            Block::default()
                                .borders(Borders::ALL)
                                .title(app.t("settings.section.update"))
            ...
            InputMode::ConfirmingDelete => {
                vec![
                    Line::from(vec![
                        ...
                        Span::styled(
                            app.current_task().map_or("", |t| &t.title),
                            Style::default()
                                .fg(theme.error)
                                .add_modifier(Modifier::BOLD),
                        ),
        ...
        match app.input_mode {
            InputMode::CreatingDescription => {
                text.insert(
                    ...
                    Line::from(vec![
                        ...
                        Span::styled(&app.wizard_title, Style::default().fg(theme.accent)),
                    ]),
                );
            ...
            InputMode::CreatingDate => {
                ...
                text.insert(
                    1,
                    Line::from(vec![
                        Span::raw(format!("{}: ", app.t("wizard.desc"))),
                        Span::styled(&app.wizard_desc, Style::default().fg(theme.muted)),
                    ]),
                );
            ...
            InputMode::CreatingReview => {
                ...
                text.insert(
                    2,
                    Line::from(vec![
                        Span::raw(format!("{}: ", app.t("wizard.date"))),
                        Span::styled(
                            &app.wizard_date,
    ...
    if app.show_help {
        ...
        let help_text = vec![
            ...
            format!(" i             : {}", app.t("help.importance_cycle")),
            format!(" o             : {}", app.t("help.sort_cycle")),
            format!(" /             : {}", app.t("hint.search")),
            format!(" d             : {}", app.t("hint.desc")),
            format!(" t             : {}", app.t("help.date_time")),
            format!(" e             : {}", app.t("hint.edit")),
            format!(" r             : {}", app.t("section.review_plan")),
            " ".to_string(),
            format!(" --- {} ---", app.t("section.pomodoro")),
            format!(" p             : {}", app.t("help.play_pause")),
            format!(" S (Shift+s)   : {}", app.t("help.skip_phase")),
            format!(" R (Shift+r)   : {}", app.t("help.reset_pomodoro")),
            format!(" f             : {}", app.t("help.force_break")),
            " ".to_string(),
            format!(" --- {} ---", "SYSTEM"),
            format!(" c             : {}", app.t("hint.settings")),
            format!(" ?             : {}", app.t("help.toggle_help")),
            format!(" q             : {}", app.t("help.quit")),
        ]
        .join("\n");
