...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let pomodoro_block = Block::default()
        ...
        .border_style(if app.active_panel == ActivePanel::Pomodoro {
            active_style
        } else {
            inactive_style
        })
    ...
    let status_text = if p.is_running {
        app.t("status.running")
    } else {
        app.t("status.paused")
    };
    ...
    f.render_widget(
        Paragraph::new(pomodoro_text).style(Style::default().fg(theme.fg)),
        layout[0],
    );
