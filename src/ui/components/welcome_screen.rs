...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let hints = Line::from(vec![
        ...
        Span::raw(if app.onboarding_index < 5 {
            format!("{} ", app.t("welcome.hint.next"))
        } else {
            format!("{} ", app.t("welcome.hint.finish"))
        }),
    ...
    f.render_widget(
        Paragraph::new(text).alignment(Alignment::Left).block(
            Block::default()
                ...
                .padding(ratatui::widgets::Padding::left(20)),
        ),
        content_area,
    );
}
...
fn draw_slide_5(f: &mut Frame, app: &App, _logo_area: Rect, content_area: Rect) {
    let text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{} min", app.settings.sync_interval_mins),
