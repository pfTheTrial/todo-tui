...
pub fn get_strings() -> HashMap<&'static str, &'static str> {
    ...
    s.insert("menu.settings.notif_pomo", "Notificaciones (Pomodoro)");
    s.insert("menu.settings.notif_tasks", "Recordatorio Diario (Tareas)");
    ...
    s.insert("sync.task.pending_push", "Push pendiente");
    s.insert("sync.task.pending_delete", "Delete pendiente");
    s.insert("sync.task.pending_pull", "Pull pendiente");
    s.insert("sync.task.failed_push", "Push con error");
    s.insert("sync.task.failed_delete", "Delete con error");
    s.insert("sync.task.failed_pull", "Pull con error");
    s.insert("sync.task.synced", "Sincronizada");
    s.insert("sync.last_success", "Último éxito");
    s.insert("sync.last_error", "Último error");
    ...
    s.insert(
        "menu.settings.update_available",
    ...
    s.insert(
        "update.unsupported",
        "Auto-update no soportado en esta plataforma",
    );
    ...
    s.insert(
        ...
        "Bienvenido a tu nuevo hub de productividad.",
    );
    s.insert(
        "welcome.splash.desc",
        "Todo-TUI está diseñado para velocidad, enfoque offline y diseño limpio.",
    );
    ...
    s.insert(
        ...
        "[1] Tareas   : Enfócate en lo que importa, organizado por prioridad.",
    );
    s.insert(
        "welcome.layout.detail",
        "[2] Detalles : Todo el contexto que necesitas de un vistazo.",
    );
    s.insert(
        "welcome.layout.pomo",
        "[3] Pomodoro : Mantén el enfoque con temporizadores integrados.",
    );
    s.insert(
        "welcome.layout.nav",
        "Cambia paneles usando [Tab] o las teclas [1, 2, 3].",
    );
    s.insert("welcome.review.title", "REVISIONES INTELIGENTES (SRS)");
    s.insert(
        "welcome.review.line1",
        "No solo termines tareas — domínalas.",
    );
    s.insert(
        "welcome.review.line2",
        "Usa el sistema de revisión para programar seguimientos periódicos",
    );
    s.insert(
        "welcome.review.line3",
        "automáticamente usando la sintaxis '1d 1s 1m'.",
    );
    s.insert(
        "welcome.review.line4",
        "Perfecto para aprendizaje, mantenimiento y hábitos a largo plazo.",
    );
    s.insert("welcome.cmd.title", "COMANDOS Y ATAJOS");
    s.insert(
        "welcome.cmd.search",
        " [/] Buscar  : Encuentra cualquier tarea al instante.",
    );
    s.insert(
        "welcome.cmd.sort",
        " [o] Ordenar : Alterna entre Prioridad, Fecha o Nombre.",
    );
    s.insert(
        "welcome.cmd.add",
        " [a] Agregar : Asistente de entrada rápida.",
    );
    s.insert(
        "welcome.cmd.config",
        " [c] Config  : Personaliza profundamente tu experiencia.",
    );
    s.insert(
        "welcome.cmd.help",
        " [?] Ayuda   : Todos los atajos listados.",
    );
    s.insert("welcome.int.title", "INTEGRACIONES");
    s.insert(
        "welcome.int.line1",
        "Todo-TUI soporta sincronización con Notion.",
    );
    s.insert(
        "welcome.int.line2",
        "Ve a Configuraciones [c] para agregar tu API Key y Database ID.",
    );
    ...
    s.insert(
        "welcome.ready.line2",
        "Tus tareas se almacenan localmente en un archivo JSON.",
    );
    s.insert(
        "welcome.ready.cta",
        "Presiona [n] para entrar a la aplicación.",
    );
