...
pub fn get_strings() -> HashMap<&'static str, &'static str> {
    ...
    s.insert("menu.settings.notif_pomo", "Notificações (Pomodoro)");
    s.insert("menu.settings.notif_tasks", "Lembrete Diário (Tarefas)");
    ...
    s.insert("sync.task.pending_push", "Push pendente");
    s.insert("sync.task.pending_delete", "Delete pendente");
    s.insert("sync.task.pending_pull", "Pull pendente");
    s.insert("sync.task.failed_push", "Push com falha");
    s.insert("sync.task.failed_delete", "Delete com falha");
    s.insert("sync.task.failed_pull", "Pull com falha");
    s.insert("sync.task.synced", "Sincronizada");
    s.insert("sync.last_success", "Último sucesso");
    s.insert("sync.last_error", "Último erro");
    s.insert("sync.retry_at", "Retry");
    ...
    s.insert(
        "update.unsupported",
        "Auto-update não suportado nesta plataforma",
    );
    ...
    s.insert(
        ...
        "Bem-vindo ao seu novo hub de produtividade.",
    );
    s.insert(
        "welcome.splash.desc",
        "Todo-TUI é projetado para velocidade, foco offline e design limpo.",
    );
    s.insert("welcome.splash.cta", "Pressione 'n' para começar o tour.");
    s.insert("welcome.layout.title", "O LAYOUT (Inspirado no Lazygit)");
    s.insert(
        "welcome.layout.tasks",
        "[1] Tarefas  : Foque no que importa, organizado por prioridade.",
    );
    s.insert(
        "welcome.layout.detail",
        "[2] Detalhes : Todo o contexto que você precisa ao seu alcance.",
    );
    s.insert(
        "welcome.layout.pomo",
        "[3] Pomodoro : Mantenha o foco com timers integrados.",
    );
    s.insert(
        "welcome.layout.nav",
        "Troque painéis usando [Tab] ou teclas [1, 2, 3].",
    );
    s.insert("welcome.review.title", "REVISÕES INTELIGENTES (SRS)");
    s.insert(
        "welcome.review.line1",
        "Não apenas termine tarefas — domine-as.",
    );
    s.insert(
        "welcome.review.line2",
        "Use o sistema de revisão para agendar follow-ups periódicos",
    );
    s.insert(
        "welcome.review.line3",
        "automaticamente usando a sintaxe '1d 1s 1m'.",
    );
    s.insert(
        "welcome.review.line4",
        "Perfeito para aprendizado, manutenção e hábitos de longo prazo.",
    );
    s.insert("welcome.cmd.title", "COMANDOS E ATALHOS");
    s.insert(
        "welcome.cmd.search",
        " [/] Buscar  : Encontre qualquer tarefa instantaneamente.",
    );
    s.insert(
        "welcome.cmd.sort",
        " [o] Ordenar : Alterne entre Prioridade, Data ou Nome.",
    );
    s.insert(
        "welcome.cmd.add",
        " [a] Adicionar: Wizard de entrada rápida.",
    );
    s.insert(
        "welcome.cmd.config",
        " [c] Config  : Personalize profundamente sua experiência.",
    );
    s.insert(
        "welcome.cmd.help",
        " [?] Ajuda   : Todos os atalhos listados.",
    );
    s.insert("welcome.int.title", "INTEGRAÇÕES");
    s.insert(
        "welcome.int.line1",
        "Todo-TUI suporta sincronização com Notion.",
    );
    s.insert(
        "welcome.int.line2",
        "Vá em Configurações [c] para adicionar sua API Key e Database ID.",
    );
    ...
    s.insert(
        "welcome.ready.line2",
        "Suas tarefas são armazenadas localmente em um arquivo JSON.",
    );
    s.insert(
        "welcome.ready.cta",
        "Pressione [n] para entrar no aplicativo.",
    );
    ...
    s.insert("help.jump", "Ir para painel");
    s.insert("help.importance_cycle", "Mudar importância");
    s.insert("help.sort_cycle", "Mudar ordenação");
    s.insert("help.date_time", "Data / Prazo");
    s.insert("help.play_pause", "Play/Pausa");
    s.insert("help.skip_phase", "Pular fase");
    s.insert("help.reset_pomodoro", "Resetar Pomodoro");
    s.insert("help.force_break", "Pausa Forçada");
    s.insert("help.toggle_help", "Mostra/Oculta ajuda");
    s.insert("help.quit", "Sair");
